...
import { Search, X, MapPin, ChevronRight, RotateCcw, CheckCircle2, Clock } from 'lucide-react'

const TYPE_COLOR = 'oklch(0.85 0.18 198)'
...
export default function RecurringProjectsPage() {
  ...
  React.useEffect(() => {
    fetch('/api/projects?type=recurring', { credentials: 'include', cache: 'no-store' })
    ...
    <div className="max-w-3xl space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl"
          style={{ background: `${TYPE_COLOR}10`, border: `1px solid ${TYPE_COLOR}30` }}>
          <RotateCcw className="h-5 w-5" style={{ color: TYPE_COLOR }} />
        </div>
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'oklch(0.92 0.008 75)' }}>定期案件</h1>
          <p className="text-xs" style={{ color: 'oklch(0.50 0.007 75)' }}>{projects.length}件</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            style={{ background: 'oklch(0.10 0.005 255 / 0.80)', border: `1px solid ${TYPE_COLOR}18`, color: 'oklch(0.88 0.008 75)' }}
          />
          {search && (
        ...
        <div className="flex gap-2">
          {(['all', 'active', 'completed'] as const).map(v => (
            <button key={v} onClick={() => setFilter(v)}
              ...
              style={filter === v
                ? { background: TYPE_COLOR, color: 'oklch(0.08 0.005 60)' }
      ...
      <div className="space-y-2.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16">
            <RotateCcw className="h-10 w-10 opacity-30 mb-3" style={{ color: TYPE_COLOR }} />
            <p className="text-sm" style={{ color: 'oklch(0.50 0.007 75)' }}>定期案件が見つかりません</p>
          </div>
        ) : (
          filtered.map(p => (
            <Link key={p.id} href={`/projects/${p.id}`}
              className="flex items-center gap-3 rounded-2xl p-4 transition-all active:scale-[0.98]"
              style={{ background: 'oklch(0.09 0.005 255 / 0.80)', border: `1px solid ${TYPE_COLOR}20` }}>
              <span className="flex h-11 w-11 items-center justify-center rounded-xl shrink-0"
                style={p.status === 'completed'
                  ...
                  : { background: `${TYPE_COLOR}10`, border: `1px solid ${TYPE_COLOR}30` }}>
                {p.status === 'completed'
                  ? <CheckCircle2 className="h-5 w-5" style={{ color: 'oklch(0.72 0.18 150)' }} />
                  : <Clock className="h-5 w-5" style={{ color: TYPE_COLOR }} />}
              </span>
              <div className="flex-1 min-w-0 space-y-0.5">
                ...
                <span className="inline-flex text-[10px] px-2 py-0.5 rounded-full font-medium"
                  style={p.status === 'completed'
                    ...
                    : { background: `${TYPE_COLOR}12`, color: TYPE_COLOR }}>
                  {p.status === 'completed' ? '完了' : '進行中'}
                </span>
              </div>
              <ChevronRight className="h-4 w-4 shrink-0" style={{ color: `${TYPE_COLOR}45` }} />
            </Link>
          ))
        )}
      </div>
