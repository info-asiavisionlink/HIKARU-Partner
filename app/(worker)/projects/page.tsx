...
export default function ProjectsPage() {
  ...
  return (
    <div className="max-w-3xl space-y-4">
      {/* タイトル */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: 'oklch(0.92 0.008 75)' }}>全案件</h1>
        ...
        <div className="flex gap-2">
          {(['all', 'active', 'completed'] as const).map(v => (
            <button key={v} onClick={() => setFilter(v)}
              ...
              style={filter === v
                ? { background: GOLD, color: 'oklch(0.08 0.005 60)' }
      ...
      <div className="space-y-2.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16">
            <Briefcase className="h-10 w-10 opacity-30 mb-3" style={{ color: GOLD }} />
            <p className="text-sm" style={{ color: 'oklch(0.50 0.007 75)' }}>案件が見つかりません</p>
          </div>
        ) : (
          filtered.map(p => {
            const { label: tLabel, color: tColor } = typeTag(p.project_type ?? '')
            return (
              <Link key={p.id} href={`/projects/${p.id}`}
                className="flex items-center gap-3 rounded-2xl p-4 transition-all active:scale-[0.98]"
                style={{ background: 'oklch(0.09 0.005 255 / 0.80)', border: `1px solid ${p.status === 'active' ? `${GOLD}25` : `${GOLD}10`}` }}>
                ...
                <div className="flex-1 min-w-0 space-y-0.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="inline-flex text-[10px] px-2 py-0.5 rounded-full font-bold"
