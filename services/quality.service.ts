...
export async function loadEvaluations(jobId: string): Promise<EvaluationRow[]> {
  const res = await fetch(`/api/ai/quality?jobId=${encodeURIComponent(jobId)}`, {
    credentials: 'include',
    cache: 'no-store',
  })
  if (!res.ok) return []
...
export function getScoreInfo(score: number) {
  if (score >= 90) return { stars: 5, label: '素晴らしい', colorClass: 'text-emerald-400', bgClass: 'bg-emerald-500/15' }
  if (score >= 75) return { stars: 4, label: '良好',       colorClass: 'text-emerald-400', bgClass: 'bg-emerald-500/15' }
  if (score >= 60) return { stars: 3, label: '普通',       colorClass: 'text-amber-400',   bgClass: 'bg-amber-500/15' }
  if (score >= 45) return { stars: 2, label: '要改善',     colorClass: 'text-amber-400',   bgClass: 'bg-amber-500/15' }
  return              { stars: 1, label: '不合格',         colorClass: 'text-red-400',     bgClass: 'bg-red-500/15' }
}
