    ...
    <form action={formAction} className="flex flex-col gap-5">
      {state.error && (
        <div className="rounded-xl p-4 text-sm"
          style={{ background: 'oklch(0.65 0.25 27 / 0.12)', border: '1px solid oklch(0.65 0.25 27 / 0.35)', color: 'oklch(0.78 0.18 30)' }}>
          {state.error}
        </div>
      )}

      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium" style={{ color: TEXT_MUTED }}>メールアドレス</label>
        <input
          name="email"
          type="email"
          placeholder="example@partner.co.jp"
          autoComplete="username"
          required
          className="w-full px-4 py-3 rounded-xl text-sm outline-none transition-all duration-200"
          style={{ background: SURFACE, border: `1px solid ${GOLD_BORDER}`, color: TEXT }}
          onFocus={(e) => { e.currentTarget.style.borderColor = GOLD; e.currentTarget.style.boxShadow = `0 0 0 3px ${GOLD_MUTED}` }}
          onBlur={(e)  => { e.currentTarget.style.borderColor = GOLD_BORDER; e.currentTarget.style.boxShadow = 'none' }}
        />
      </div>

      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium" style={{ color: TEXT_MUTED }}>パスワード</label>
