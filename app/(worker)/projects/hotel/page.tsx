...
import { Search, X, MapPin, ChevronRight, Hotel, CheckCircle2, Clock } from 'lucide-react'

const TYPE_COLOR = 'oklch(0.75 0.15 290)'
...
export default function HotelProjectsPage() {
  ...
  React.useEffect(() => {
    fetch('/api/projects?type=hotel', { credentials: 'include', cache: 'no-store' })
    ...
    <div className="max-w-3xl space-y-4">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl"
          ...
          <Hotel className="h-5 w-5" style={{ color: TYPE_COLOR }} />
        </div>
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'oklch(0.92 0.008 75)' }}>ホテル案件</h1>
      ...
      <div className="space-y-2.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16">
            <Hotel className="h-10 w-10 opacity-30 mb-3" style={{ color: TYPE_COLOR }} />
            <p className="text-sm" style={{ color: 'oklch(0.50 0.007 75)' }}>ホテル案件が見つかりません</p>
          </div>
        ) : (
          filtered.map(p => (
