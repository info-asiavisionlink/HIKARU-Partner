...
import { Search, X, MapPin, ChevronRight, Zap, CheckCircle2, Clock } from 'lucide-react'

const GOLD = 'oklch(0.73 0.12 78)'
const TYPE_COLOR = GOLD
...
export default function SpotProjectsPage() {
  ...
  React.useEffect(() => {
    fetch('/api/projects?type=spot', { credentials: 'include', cache: 'no-store' })
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'oklch(0.92 0.008 75)' }}>単発案件</h1>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            style={{ background: 'oklch(0.10 0.005 255 / 0.80)', border: `1px solid ${GOLD}18`, color: 'oklch(0.88 0.008 75)' }}
          />
          {search && (
      ...
      <div className="space-y-2.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16">
            <Zap className="h-10 w-10 opacity-30 mb-3" style={{ color: TYPE_COLOR }} />
            <p className="text-sm" style={{ color: 'oklch(0.50 0.007 75)' }}>単発案件が見つかりません</p>
          </div>
        ) : (
          filtered.map(p => (
